...
De-Tokenize
∘ ∘ ∘
Internet-Scale VQA + Robot Action Data Closed-Loop 
Robot Control
...
them on robotic trajectories bytokenizing the actions into text tokensand creating “multimodal
...
2. Related Work
Vision-language models.There are several categories ofVision-Language Models(VLMs) (Gan et al.,
...
associate tokens from the model’sexisting tokenization with the discrete action bins. This requires
5
RT-2: Vision-Language-Action Models Transfer Web Knowledge to Robotic Control
...
that improves robot performance isco-fine-tuning robotics data with the original web data instead of
...
specific instantiations of RT-2 that leverage pre-trained VLMs: (1)RT-2-PaLI-Xis built from 5B and
55B PaLI-X (Chen et al., 2023a), and (2)RT-2-PaLM-Eis built from 12B PaLM-E (Driess et al., 2023).
...
state-of-the-art policy, we useRT-1(Brohan et al., 2022), a 35M parameter transformer-based model.
To compare against state-of-the-art pretrained representations, we useVC-1 (Majumdar et al., 2023a)
and R3M (Nair et al., 2022b), with policies implemented by training an RT-1 backbone to take their
representations as input. To compare against other architectures for using VLMs, we useMOO (Stone
...
backgrounds, and environments?
(c) Unseen Environments(a) Unseen Objects
...
seen taskscategory, we use the same suite of seen instructions as in RT-1 (Brohan et al., 2022), which
...
7
RT-2: Vision-Language-Action Models Transfer Web Knowledge to Robotic Control
Figure 3 shows example generalization evaluations, which are split intounseen categories (objects,
backgroundsand environments), and are additionally split into easy and hard cases. For unseen objects,
...
format “X Y ”, whereX and Y range between {-10, -9, ..., +9, +10}, and represent delta 2D cartesian
...
manet, 2020).
in the robot data by transferring knowledge from the web. We refer to such capabilities asemergent, in
the sense that they emerge by transferring Internet-scale pretraining. We do not expect such transfer
to enable new roboticmotions, but we do expect semantic and visual concepts, including relations
...
al vaso verde”). We refer to the last category ashuman recognitiontasks, which include tasks such as
...
aspects of these models, we remove theseen tasksevaluation from this set of experiments.
...
over semantic and visual concepts, the robot does not acquire any ability to perform newmotions
...
S. Dasari and A. Gupta. Transformers for one-shot visual imitation. InConference on Robot Learning,
pages 2071–2084. PMLR, 2021.
S. Dasari, F. Ebert, S. Tian, S. Nair, B. Bucher, K. Schmeckpeper, S. Singh, S. Levine, and C. Finn.
Robonet: Large-scale multi-robot learning. InConference on Robot Learning, 2019.
13
RT-2: Vision-Language-Action Models Transfer Web Knowledge to Robotic Control
...
C. Finn and S. Levine. Deep visual foresight for planning robot motion. In2017 IEEE International
...
actionable knowledge for embodied agents. InInternational Conference on Machine Learning, pages
9118–9147. PMLR, 2022.
S. James, M. Bloesch, and A. J. Davison. Task-embedded control networks for few-shot imitation
learning. InConference on robot learning, pages 783–795. PMLR, 2018.
E. Jang, A. Irpan, M. Khansari, D. Kappler, F. Ebert, C. Lynch, S. Levine, and C. Finn. Bc-z: Zero-
shot task generalization with robotic imitation learning. InConference on Robot Learning, pages
991–1002. PMLR, 2021.
...
learning. InInternational Conference on Machine Learning, pages 5639–5650. PMLR, 2020b.
...
from offline data and crowd-sourced annotation. InConference on Robot Learning, pages 1303–1315.
PMLR, 2022a.
...
hours. In2016 IEEE international conference on robotics and automation (ICRA), pages 3406–3413.
IEEE, 2016.
...
J. Clark, et al. Learning transferable visual models from natural language supervision. InInterna-
...
Conference on Robot Learning, volume 205 ofProceedings of Machine Learning Research, pages 492–
504. PMLR, 14–18 Dec 2023. URLhttps://proceedings.mlr.press/v205/shah23b.html.
...
Progprompt: Generating situated robot task plans using large language models. InICRA, 2023.
M. H. Smith and L. S. Coles. Design of a low cost, general purpose robot. InIJCAI, pages 324–336,
1973.
...
experimental dataset of planar pushing. In2016 IEEE/RSJ international conference on intelligent
...
X. Zhai, A. Kolesnikov, N. Houlsby, and L. Beyer. Scaling vision transformers. InProceedings of the
...
a natural language instruction from one of seven skills: "PickObject", "MoveObject Near Object",
"Place Object Upright", "KnockObject Over", "OpenDrawer", "CloseDrawer", "PlaceObject into
Receptacle", and "PickObject from Receptacle and place on the counter". Further details can
...
can accept sequences of𝑛 images, leading to𝑛× 𝑘 tokens per image, where𝑘 is the number of patches
...
Recognition
(a) Reasoning
“pick a healthy drink”“move banna near the 
sum of two plus one”
“déplacer les frites verts 
dans la tasse rouge”
“move coke can
near Y”
“move apple to tree”“move banana to 
android”
“put coke can close
to dog”
“move coke can to 
...
RT-2 model not generalizing tounseen object dynamics. In these cases, although the model is able
...
Figure 9| Qualitative example failure cases in the real-world failing to generalize tounseen object dynamics.
...
VC-1 (Majumdar et al., 2023a) 7 25 0 11 0 8 20 13 10 20 7 13 11RT-1 (Brohan et al., 2022) 27 20 0 16 5 0 32 28 16 20 20 20 17RT-2-PaLI-X-55B (ours)93 60 93 82 25 52 48 58 46 53 53 53 60RT-2-PaLM-E-12B (ours) 67 20 20 36 35 56 44 35 43 33 53 43 40
...
scenarios if shown in Figure 3. 26
